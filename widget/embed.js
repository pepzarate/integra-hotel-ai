(function () {
  ...
  `;
    ...
    function parseMarkdown(text) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('- ') || line.startsWith('• ')) {
                ...
                if (line === '') { html += '<br>'; }
                else { html += '<p style="margin:3px 0;">' + line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') + '</p>'; }
            }
        }
        if (inList) html += '</ul>';
        return html;
    ...
    function toggleWidget() {
        isOpen = !isOpen;
        document.getElementById('sofia-window').classList.toggle('open', isOpen);
        document.getElementById('sofia-toggle').classList.toggle('open', isOpen);
        document.getElementById('sofia-badge').classList.remove('visible');
    ...
    async function sendMessage() {
        ...
        try {
            ...
            hideTyping();
            appendMessage('sofia', 'Hubo un problema de conexión. Por favor intenta de nuevo.');
        }
    }
    ...
    function showTyping() {
        ...
        row.id = 'sofia-typing-row';
        const avatar = document.createElement('div');
        avatar.className = 'sofia-msg-avatar';
        avatar.textContent = CONFIG.avatar;
        const indicator = document.createElement('div');
        indicator.className = 'sofia-typing';
        indicator.innerHTML = '<div class="sofia-dot"></div><div class="sofia-dot"></div><div class="sofia-dot"></div>';
        row.appendChild(avatar);
    ...
    function hideTyping() {
        ...
        document.getElementById('sofia-input').focus();
        const row = document.getElementById('sofia-typing-row');
        if (row) row.remove();
    }

    function scrollToBottom() {
        const m = document.getElementById('sofia-messages');
